...
function initImageCrossfade(hoverImgSelector, overlaySelector) {
  ...
  overlay.addEventListener("mouseenter", () => {
    hoverImg.style.opacity = 1; // fade in gambar hover
  });

  overlay.addEventListener("mouseleave", () => {
    hoverImg.style.opacity = 0; // fade out kembali ke default
  });
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (revealAnimation) {
    ...
    // If animations are disabled, hide overlays immediately and show content
    document.querySelector(".first").style.top = "-100%";
    document.querySelector(".second").style.top = "-100%";
    document.querySelector(".third").style.top = "-100%";

    // Show elements immediately
    const elementsToShow = document.querySelectorAll(
      ".home_img, .home__information, .anime-text, .nav__logo, .nav__item, .home__social-icon"
    );
